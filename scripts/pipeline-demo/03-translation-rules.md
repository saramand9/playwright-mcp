...
**判断标准（代码层面）**：
- `page.getByText('xxx')` / `expect(page).toHaveURL(...)` — 无 selector 绑定 → 裸 expect
- `page.locator('#xxx').getByText(...)` / `page.getByRole('table')` — 绑定了 selector → robustLocate
